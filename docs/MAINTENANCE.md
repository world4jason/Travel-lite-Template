...
The harness serves MapLibre from the pinned `maplibre-gl` devDependency instead of the CDN, and the Map view is checked only after the map has initialised or fallen back. When `runtime-features.js` moves to a new MapLibre version, bump the devDependency to the same version; the spec refuses to run if they differ.
